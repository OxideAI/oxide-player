...
function openDb(): Promise<IDBDatabase> {
  return new Promise((resolve, reject) => {
    ...
    request.onerror = () =>
      reject(request.error ?? new Error("Unable to open library cache"));
  });
}
...
export async function readLibraryCache(): Promise<LibraryCacheSnapshot | null> {
  ...
  try {
    ...
    const stored = await new Promise<StoredSnapshot | undefined>(
      (resolve, reject) => {
        const request = db
...
export async function removeTrackFromLibraryCache(
  trackId: number,
): Promise<void> {
  const snapshot = await readLibraryCache();
  if (
    snapshot === null ||
    !snapshot.tracks.some((track) => track.id === trackId)
  )
    return;
