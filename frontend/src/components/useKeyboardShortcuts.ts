      ...
      switch (action) {
        case "togglePlay":
          fire(
            "togglePlay",
            h.onTogglePlay,
            status?.state === "playing" ? "⏸ Paused" : "▶ Playing",
          );
          return;
        case "volumeUp":
        case "volumeDown": {
          ...
          const v = Math.max(
            ...
            Math.min(
              100,
              cur + (action === "volumeUp" ? VOLUME_STEP : -VOLUME_STEP),
            ),
          );
        ...
        case "seekFwd": {
          ...
          const t = Math.max(
            0,
            cur + (action === "seekFwd" ? SEEK_STEP : -SEEK_STEP),
          );
        ...
        case "toggleShuffle":
          fire(
            "toggleShuffle",
            h.onToggleShuffle,
            status?.random ? "Shuffle off" : "Shuffle on",
          );
          return;
        case "toggleMute":
          fire(
            "toggleMute",
            h.onToggleMute,
            status?.volume != null && status.volume === 0 ? "Unmuted" : "Muted",
          );
          return;
        case "help":
