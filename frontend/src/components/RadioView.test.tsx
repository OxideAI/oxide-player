...
const listRadio = vi.fn<() => Promise<RadioStation[]>>(() =>
  Promise.resolve([]),
);
const addRadio = vi.fn<
  (name: string, url: string, artwork: string | null) => Promise<RadioStation>
>(() => Promise.reject(new Error("unexpected addRadio call")));
const updateRadio = vi.fn<
  (id: string, name: string, artwork: string | null) => Promise<RadioStation>
>(() => Promise.reject(new Error("unexpected updateRadio call")));
const deleteRadio = vi.fn<(id: string) => Promise<unknown>>(() =>
  Promise.resolve({}),
);
const playRadio = vi.fn<(id: string) => Promise<unknown>>(() =>
  Promise.resolve({}),
);
...
describe("RadioView", () => {
  ...
  it("rejects a non-http(s) URL without calling the api", async () => {
    ...
    await waitFor(() => expect(getByText("JFK Ibiza")).toBeTruthy());
    fireEvent.change(getByLabelText("Station name"), {
      target: { value: "Nope" },
    });
    fireEvent.change(getByLabelText("Stream URL"), {
      target: { value: "ftp://x.example/" },
    });
    fireEvent.click(getByText("Add station"));
    await waitFor(() =>
      expect(
        getByText("Station URL must start with http:// or https://"),
      ).toBeTruthy(),
    );
  ...
  it("rejects an empty name without calling the api", async () => {
    ...
    fireEvent.click(getByText("Add station"));
    await waitFor(() =>
      expect(getByText("Station name is required.")).toBeTruthy(),
    );
    expect(addRadio).not.toHaveBeenCalled();
  });
  ...
  it("rejects invalid artwork URLs without calling the api", async () => {
    ...
    await waitFor(() =>
      expect(
        getByText("Artwork URL must start with http:// or https://"),
      ).toBeTruthy(),
    );
  ...
  it("disables Play and shows Playing for the live station", async () => {
    const { getByText } = render(
      <RadioView
        nowPlayingUri="https://stream.aiir.com/7dsjltmny8cvv"
        isPlaying
      />,
    );
  ...
  it("renders the empty state when there are no stations", async () => {
    ...
    const { getByText } = render(<RadioView {...noopProps} />);
    await waitFor(() =>
      expect(getByText("No stations yet — add one above.")).toBeTruthy(),
    );
  });
