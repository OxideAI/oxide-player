...
describe('EqGraph', () => {
  it('renders an svg with the response curve and axes when bands present', () => {
    ...
    // Response curve path
    const curve = container.querySelector('path[class*="curve"]') as SVGPathElement | null
    expect(curve).not.toBeNull()
    expect(curve!.getAttribute('d')).toMatch(/^M/)
    expect((curve!.getAttribute('d') || '').split(' ').length).toBeGreaterThan(10)
    ...
    expect(container.querySelectorAll('text[class*="tickLbl"]').length).toBeGreaterThan(8)
  })
  ...
  it('renders a flat curve when there are no bands', () => {
    ...
    const ys = (d.match(/,(\d+\.\d+)/g) || []).map((s) => parseFloat(s.slice(1)))
