...
describe('VisualizationSettings', () => {
  ...
  it.each([
    ...
    render(<VisualizationSettings config={{ ...config, visualizer_fft: state !== 'disabled' }} onSave={vi.fn()} />)
    expect((await screen.findByTestId('visualizer-status')).textContent).toContain(copy)
    if (state === 'waiting-for-capture') expect(screen.getByText(/non-terminal/i)).toBeTruthy()
    if (state === 'startup/runtime-error') expect(screen.getByText(/invalid capture source/i)).toBeTruthy()
  })

  it('saves enablement through the supplied config boundary and keeps style tuning separate', async () => {
    const onSave = vi.fn().mockResolvedValue({ ...config, visualizer_fft: true })
    render(<VisualizationSettings config={config} onSave={onSave} />)
    fireEvent.click(screen.getByRole('checkbox', { name: /Enable visualization/i }))
    fireEvent.click(screen.getByRole('button', { name: /Save visualization/i }))
    await waitFor(() => expect(onSave).toHaveBeenCalledWith(true))
    expect((screen.getByRole('checkbox', { name: /Enable visualization/i }) as HTMLInputElement).checked).toBe(true)
  })
  ...
  it('keeps a failed draft and cancel restores the saved value', async () => {
    ...
    expect((screen.getByRole('checkbox', { name: /Enable visualization/i }) as HTMLInputElement).checked).toBe(false)
  })
})
