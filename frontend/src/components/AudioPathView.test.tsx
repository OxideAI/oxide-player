...
describe('AudioPathView route reduction', () => {
  it.each([
    ...
    expect(reduceAudioPath({ status: status({ state: playerState }), outputs: [output()], connected: true }).state).toBe(expected)
  })

  it('prioritizes stale status over an MPD error after disconnect', () => {
    expect(reduceAudioPath({
  ...
  it('surfaces MPD errors while the status connection is healthy', () => {
    expect(reduceAudioPath({
      ...
      outputs: [output()],
      connected: true,
    }).state).toBe('mpd-error')
  })

  it('distinguishes unavailable, zero-output, and multiple-output states', () => {
    expect(reduceAudioPath({ status: null, outputs: [], connected: false }).state).toBe('unavailable')
    expect(reduceAudioPath({ status: status(), outputs: [], connected: true }).state).toBe('zero-output')
    expect(reduceAudioPath({
      status: status({ outputs: [
        { id: 1, name: 'USB DAC', enabled: true },
        { id: 2, name: 'Bedroom', enabled: true },
      ] }),
      outputs: [output(), output({ id: 2, name: 'Bedroom', selection_key: 'alsa:Bedroom' })],
      connected: true,
    }).state).toBe('multiple-output')
  })

  it('keeps route health visible from a live snapshot while details are unavailable', () => {
    const result = reduceAudioPath({ status: status(), outputs: [], connected: true, detailsReady: false })
  ...
  it('uses stable selection identity rather than a numeric runtime id', () => {
    ...
    expect(stableOutputIdentity(output({ id: 99 }))).toBe(stableOutputIdentity(output({ id: 4 })))
  })
})
